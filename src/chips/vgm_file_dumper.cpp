...
extern "C"
{
    ADLMIDI_EXPORT void adl_set_vgm_out_path(const char *path)
    {
        g_vgm_path = path;
    }
}
...
void VGMFileDumper::writeHead()
{
    ...
    g_write_le(m_output, m_vgm_head.clock_segapcm);
    g_write_le(m_output, m_vgm_head.interface_spcm);
    g_write_le(m_output, m_vgm_head.clock_rf5c68);
    g_write_le(m_output, m_vgm_head.clock_ym2203);
    g_write_le(m_output, m_vgm_head.clock_ym2608);
    g_write_le(m_output, m_vgm_head.clock_ym2610b);
    g_write_le(m_output, m_vgm_head.clock_ym3812);
    g_write_le(m_output, m_vgm_head.clock_ym3526);
    g_write_le(m_output, m_vgm_head.clock_y8950);
    g_write_le(m_output, m_vgm_head.clock_ymf262);
    g_write_le(m_output, m_vgm_head.clock_ymf278);
    g_write_le(m_output, m_vgm_head.clock_ymf271);
    g_write_le(m_output, m_vgm_head.clock_ymz280b);
    g_write_le(m_output, m_vgm_head.clock_rf5c164);
    g_write_le(m_output, m_vgm_head.clock_pwm);
    g_write_le(m_output, m_vgm_head.clock_ay8910);
    //g_write_le(m_output, m_vgm_head.chip_type_ay8910);
    std::fwrite(&m_vgm_head.flags_ym2203, 1, 1, m_output);
    std::fwrite(&m_vgm_head.flags_ym2608, 1, 1, m_output);
}
...
{
    ADLChipBaseBufferedT::setRate(rate, clock);
    m_actual_rate = isRunningAtPcmRate() ? rate : nativeRate();
    m_vgm_head.clock_ymf262 = m_clock;
}
...
void VGMFileDumper::writeReg(uint32_t port, uint16_t addr, uint8_t data)
{
    ...
    if(port >= 4u)
        return; // VGM DOESN'T SUPPORT MORE THAN 2 CHIPS

    if(port >= 2u && ((m_vgm_head.clock_ymf262 & 0x40000000) == 0))
        m_vgm_head.clock_ymf262 |= 0x40000000;

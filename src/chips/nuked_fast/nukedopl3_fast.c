...
void OPL3Fast_Generate4Ch(opl3_chip *chip, int16_t *buf4)
{
    ...
    while ((writebuf = &chip->writebuf[chip->writebuf_cur]), writebuf->time <= chip->writebuf_samplecnt)
    {
        ...
        writebuf->reg &= 0x1ff;
        OPL3Fast_WriteReg(chip, writebuf->reg, writebuf->data);
...
{
    int16_t samples[4];
    OPL3Fast_Generate4Ch(chip, samples);
    buf[0] = samples[0];
    buf[1] = samples[1];
}
...
void OPL3Fast_Generate4ChResampled(opl3_chip *chip, int16_t *buf4)
{
    while (chip->samplecnt >= chip->rateratio)
    {
        ...
        OPL3Fast_Generate4Ch(chip, chip->samples);
...
{
    int16_t samples[4];
    OPL3Fast_Generate4ChResampled(chip, samples);
    buf[0] = samples[0];
    buf[1] = samples[1];
}
...
void OPL3Fast_WritePan(opl3_chip *chip, uint16_t reg, uint8_t v)
{
    ...
    uint8_t data = v & 0x7f;
    opl3_channel *channel = &chip->channel[9 * high + (regm & 0x0f)];
...
void OPL3Fast_WriteRegBuffered(opl3_chip *chip, uint16_t reg, uint8_t v)
{
    ...
    if (writebuf->reg & 0x200)
    {
        OPL3Fast_WriteReg(chip, writebuf->reg & 0x1ff, writebuf->data);
...
void OPL3Fast_Generate4ChStream(opl3_chip *chip, int16_t *sndptr1, int16_t *sndptr2, uint32_t numsamples)
{
    uint_fast32_t i;
    int16_t samples[4];
...
void OPL3Fast_GenerateStream(opl3_chip *chip, int16_t *sndptr, uint32_t numsamples)
{
    ...
    for(i = 0; i < numsamples; i++)
    {
        OPL3Fast_GenerateResampled(chip, sndptr);
        sndptr += 2;
    }
}
